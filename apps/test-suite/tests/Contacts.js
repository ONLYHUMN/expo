...
export async function test({ describe, it, xdescribe, jasmine, expect, afterAll }) {
  ...
  describeWithPermissions('Contacts', () => {
    ...
    it('Contacts.createContactAsync() with starred', async () => {
      ...
      const contact = await Contacts.getContactByIdAsync(contactId, [Contacts.Fields.Starred]);
      expect(contact.starred).toBe(isAndroid ? true : undefined);
    });
    ...
    it('Contacts.getContactByIdAsync() checks shape of all results', async () => {
      ...
      contacts.data.forEach(({ id, name, phoneNumbers, emails, starred }) => {
        ...
        expect(starred).toBe(isAndroid ? false : undefined);
      });
    });
    ...
    if (isAndroid) {
      ...
      it('Contacts.getContactsAsync() returns starred contacts', async () => {
        ...
        const filteredContacts = contacts.filter((contact) => contact.starred);
